...
def index():
    engine = pyttsx3.init()
    engine.say("Welcome to the automated interview. This interview is designed to ask you a series of questions and record your responses. Click the Start Assesment below to begin.")
    engine.runAndWait()
    return render_template('index2.html')
    


def score_the_studet():
    # Compare the transcribed text with expected answers
    for answer in expected_answers:
        similarity_score = SequenceMatcher(None, answer, transcript).ratio()
...
@app.route('/start_interview',methods=['POST', 'GET'])
def start_interview():
